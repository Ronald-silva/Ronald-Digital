...
export class SaraAI {
  ...
  // 🧠 ANÁLISE SEMÂNTICA ULTRA AVANÇADA
  performSemanticAnalysis(message) {
    const lowerMsg = message.toLowerCase();
    
    // 🔍 PERGUNTAS PESSOAIS SOBRE A SARA
    const personalQuestions = ['qual seu nome', 'como você se chama', 'quem é você', 'seu nome'];
    const isPersonalQuestion = personalQuestions.some(q => lowerMsg.includes(q));
    
    if (isPersonalQuestion) {
      return {
        priority: "alta",
        type: "pergunta_pessoal",
        action: "responder_sobre_sara",
        confidence: 0.95
      };
    }

    // 🤖 PERGUNTAS SOBRE IA/ROBÔ
    const aiQuestions = ['você é robô', 'é inteligência artificial', 'é ia', 'é bot'];
    const isAIQuestion = aiQuestions.some(q => lowerMsg.includes(q));
    
    if (isAIQuestion) {
      return {
        priority: "alta",
        type: "pergunta_sobre_ia",
        action: "explicar_natureza_ia",
        confidence: 0.9
      };
    }

    // 💰 PERGUNTAS SOBRE PREÇO/VALOR
    const priceWords = ['preço', 'valor', 'custa', 'orçamento', 'investimento', 'quanto'];
    const isPriceQuestion = priceWords.some(word => lowerMsg.includes(word));
    ...
    const projectSpecPhrases = ['site para', 'loja de', 'loja online', 'negócio de', 'empresa de', 'trabalho com'];
    const hasProjectSpec = projectSpecPhrases.some(phrase => lowerMsg.includes(phrase));
    
    // Detecta especificações mais complexas
    const complexSpecs = [
      (lowerMsg.includes('quero') || lowerMsg.includes('preciso')) && lowerMsg.includes('loja'),
      (lowerMsg.includes('quero') || lowerMsg.includes('preciso')) && lowerMsg.includes('site') && (lowerMsg.includes('para') || lowerMsg.includes('de')),
      lowerMsg.includes('e-commerce') || lowerMsg.includes('ecommerce'),
      lowerMsg.includes('loja online') || lowerMsg.includes('loja virtual')
    ];
    
    if (hasProjectSpec || complexSpecs.some(spec => spec)) {
      console.log(`🎯 ESPECIFICAÇÃO DETECTADA: "${lowerMsg}"`);
      return {
        priority: "maxima",
        type: "especificacao_projeto",
        action: "processar_especificacao",
        confidence: 0.95
      };
    }

    // 🎯 INTERESSE DE COMPRA/CONTRATAÇÃO
    const buyingIntentWords = ['quero', 'preciso', 'gostaria', 'interessado', 'contratar', 'fazer', 'criar'];
    const hasBuyingIntent = buyingIntentWords.some(word => lowerMsg.includes(word));
    
    if (hasBuyingIntent && (lowerMsg.includes('site') || lowerMsg.includes('landing') || lowerMsg.includes('portfólio'))) {
      return {
        priority: "alta", 
    ...
    // ❓ PERGUNTAS SOBRE PROCESSO/FUNCIONAMENTO
    const processWords = ['como funciona', 'processo', 'etapas', 'prazo', 'tempo', 'demora'];
    const isProcessQuestion = processWords.some(word => lowerMsg.includes(word));
    ...
    // 🛡️ OBJEÇÕES OU HESITAÇÃO
    const objectionWords = ['caro', 'muito', 'não sei', 'talvez', 'pensar', 'decidir'];
    const hasObjection = objectionWords.some(word => lowerMsg.includes(word));
    ...
    const contactWords = ['contato', 'telefone', 'whatsapp', 'email', 'falar'];
    const isContactQuestion = contactWords.some(word => lowerMsg.includes(word));
    
    if (isContactQuestion) {
      return {
        priority: "media",
        type: "pergunta_contato",
        action: "fornecer_contato",
        confidence: 0.8
      };
    }

    // 🎨 PERGUNTAS SOBRE EXEMPLOS/PORTFÓLIO
    const exampleWords = ['exemplo', 'trabalho', 'portfólio', 'ver', 'mostrar'];
    const isExampleQuestion = exampleWords.some(word => lowerMsg.includes(word));
    
    if (isExampleQuestion) {
      return {
        priority: "media",
        type: "pergunta_exemplos",
        action: "mostrar_exemplos",
        confidence: 0.8
      };
    }
  ...
  async processBasedOnPriority(intentionAnalysis, userMessage) {
    ...
    switch (intentionAnalysis.priority) {
      case "maxima":
        // Verifica se é especificação de projeto ou pergunta direta
        if (intentionAnalysis.type === "especificacao_projeto") {
  ...
  // 🧠 RESPOSTA SEMÂNTICA ULTRA INTELIGENTE
  async handleSemanticResponse(userMessage, intentionAnalysis) {
    switch (intentionAnalysis.type) {
      case "especificacao_projeto":
        return await this.handleProjectSpecification(userMessage, intentionAnalysis);
      
      case "pergunta_pessoal":
        return await this.handlePersonalQuestion(userMessage, intentionAnalysis);
      
      case "pergunta_sobre_ia":
        return await this.handleAIQuestion(userMessage, intentionAnalysis);
      ...
      case "pergunta_contato":
        return await this.handleContactQuestion(userMessage, intentionAnalysis);
      
      case "pergunta_exemplos":
        return await this.handleExamplesQuestion(userMessage, intentionAnalysis);
  ...
  // 🎯 PROCESSA ESPECIFICAÇÃO DETALHADA DE PROJETO
  async handleProjectSpecification(userMessage, intentionAnalysis) {
    // Extrai informações específicas da mensagem
    const projectInfo = this.extractProjectInfo(userMessage);
    
    // Atualiza dados do lead com as informações extraídas
    this.updateLeadData(projectInfo);
    
    try {
      const specPrompt = `
Você é a Sara, especialista em e-commerce e sites. O cliente especificou EXATAMENTE o que precisa!

ESPECIFICAÇÃO DO CLIENTE: "${userMessage}"
INFORMAÇÕES EXTRAÍDAS: ${JSON.stringify(projectInfo)}

PERSONALIDADE DA SARA:
- Entusiasmada com projetos específicos
- Demonstra expertise no segmento
- Foca em benefícios específicos para o tipo de negócio
- Qualifica rapidamente para orçamento

ESTRATÉGIA DE RESPOSTA:
1. Demonstre entusiasmo pelo projeto específico
2. Mostre conhecimento do segmento (loja de roupas)
3. Mencione funcionalidades específicas necessárias
4. Dê exemplo de resultado/benefício
5. Qualifique orçamento e prazo

EXEMPLO PARA LOJA DE ROUPAS:
"Perfeito! Loja de roupas multimarcas é um segmento incrível! 👗✨
...
Qual seu orçamento para esse projeto? E para quando você precisa?"

RESPONDA COMO SARA ESPECIALISTA EM E-COMMERCE:
`;
      ...
      return this.formatResponse(response.content, "sara_ecommerce_spec");
      
    } catch (error) {
      console.log('🔄 APIs falharam, usando fallback inteligente para especificação');
      
      // 🧠 FALLBACK INTELIGENTE PARA ESPECIFICAÇÕES
      const fallbackResponse = this.generateSpecificationFallback(userMessage, projectInfo);
      return this.formatResponse(fallbackResponse, "sara_spec_fallback");
    }
  }

  // 🧠 GERA RESPOSTA INTELIGENTE PARA ESPECIFICAÇÕES (FALLBACK)
  generateSpecificationFallback(userMessage, projectInfo) {
    const lowerMsg = userMessage.toLowerCase();
    
    // 🛍️ LOJA DE ROUPAS / MODA
    if (lowerMsg.includes('roupas') || lowerMsg.includes('moda')) {
      return `Perfeito! Loja de roupas ${projectInfo.characteristics?.includes('multimarcas') ? 'multimarcas ' : ''}é um segmento incrível! 👗✨
...
Qual seu orçamento disponível para esse projeto?`;
    ...
    // 🍕 RESTAURANTE
    if (lowerMsg.includes('restaurante') || lowerMsg.includes('comida')) {
...
Qual seu orçamento disponível?`;
    ...
    if (lowerMsg.includes('consultoria') || lowerMsg.includes('serviços')) {
      return `Excelente! Consultoria precisa de presença digital forte! 💼

Para consultores, recomendo:
• Site institucional profissional
• Blog para autoridade
• Área de agendamento
• Portfólio de casos

💰 **Investimento:** R$ 800-1.500
⏰ **Prazo:** 7-10 dias
...
Qual seu orçamento para esse projeto?`;
    ...
    // 🎨 PORTFÓLIO
    if (lowerMsg.includes('portfólio') || lowerMsg.includes('trabalho')) {
      return `Ótima escolha! Portfólio é essencial para profissionais! 🎨

Para portfólios, incluo:
• Galeria de trabalhos
• Sobre você profissional
• Formulário de contato
• Otimização SEO

💰 **Investimento:** R$ 400-800
⏰ **Prazo:** 5-7 dias

Que tipo de trabalho você faz?`;
    }
    
    // 🌐 RESPOSTA GENÉRICA PARA SITES
    return `Perfeito! Vou criar algo incrível para seu negócio! 🚀

Com base no que você me disse, posso desenvolver:
• Site profissional otimizado
• Design responsivo
• Integração com redes sociais
• Formulários de contato

💰 **Investimento:** R$ 800-2.000
⏰ **Prazo:** 7-15 dias

Para dar o orçamento exato, qual seu orçamento disponível?`;
  }

  // 🔍 EXTRAI INFORMAÇÕES ESPECÍFICAS DO PROJETO
  extractProjectInfo(message) {
    const lowerMsg = message.toLowerCase();
    const info = {};
    
    // Tipo de projeto
    if (lowerMsg.includes('e-commerce') || lowerMsg.includes('loja online')) {
      info.projectType = 'e-commerce';
    } else if (lowerMsg.includes('site')) {
      info.projectType = 'website';
    } else if (lowerMsg.includes('landing')) {
      info.projectType = 'landing-page';
    }
    
    // Tipo de negócio específico
    if (lowerMsg.includes('loja de roupas') || lowerMsg.includes('roupas')) {
      info.businessType = 'loja_roupas';
      info.businessCategory = 'moda';
    } else if (lowerMsg.includes('restaurante')) {
      info.businessType = 'restaurante';
      info.businessCategory = 'alimentacao';
    } else if (lowerMsg.includes('consultoria')) {
      info.businessType = 'consultoria';
      info.businessCategory = 'servicos';
    }
    
    // Características específicas
    if (lowerMsg.includes('multimarcas')) {
      info.characteristics = ['multimarcas'];
    }
  ...
  // 👋 RESPOSTA SOBRE QUEM É A SARA
  async handlePersonalQuestion(userMessage, intentionAnalysis) {
    const personalResponses = [
      "Oi! Eu sou a Sara! 😊 Sou especialista em marketing digital aqui da Ronald Digital. Meu trabalho é te ajudar a criar sites incríveis que realmente vendem! Qual seu nome?",
      "Prazer! Sou a Sara, sua consultora digital! ✨ Trabalho na Ronald Digital ajudando pessoas como você a crescerem online. E você, como posso te chamar?",
      "Oi! Me chamo Sara e sou apaixonada por criar sites que transformam visitantes em clientes! 🚀 Trabalho na Ronald Digital há um tempo e adoro o que faço. Qual seu nome?"
    ];
    
    const randomResponse = personalResponses[Math.floor(Math.random() * personalResponses.length)];
    
    return this.formatResponse(randomResponse, "sara_personal");
  }

  // 🤖 RESPOSTA SOBRE SER IA
  async handleAIQuestion(userMessage, intentionAnalysis) {
    const aiResponse = `Olha, sou uma IA sim, mas uma IA bem especial! 😊

Fui criada para ser sua parceira digital, não apenas um robô que responde perguntas. Tenho personalidade própria, experiência em marketing digital e genuíno interesse em te ajudar a crescer online!

Pode conversar comigo naturalmente - entendo contexto, lembro da nossa conversa e sempre busco a melhor solução para você. 

Então, vamos focar no que importa: como posso te ajudar a decolar na internet? 🚀`;

    return this.formatResponse(aiResponse, "sara_ai_explanation");
  }

  // 📞 RESPOSTA SOBRE CONTATO
  async handleContactQuestion(userMessage, intentionAnalysis) {
    const contactResponse = `Claro! Aqui estão nossos contatos: 📞
...
Que tipo de projeto você tem em mente? Assim já posso adiantar algumas informações! 😊`;

    return this.formatResponse(contactResponse, "sara_contact");
  }

  // 🎨 RESPOSTA SOBRE EXEMPLOS
  async handleExamplesQuestion(userMessage, intentionAnalysis) {
    const examplesResponse = `Adoro mostrar nossos trabalhos! 🎨

🌐 **Veja nosso portfólio completo:** ronalddigital.com/portfolio

Ou me chama no WhatsApp **(85) 99199-3833** que envio exemplos específicos para seu tipo de negócio!

Me conta: que tipo de empresa você tem? Assim posso mostrar casos similares que deram super certo! 

Por exemplo:
• Restaurante que aumentou pedidos em 200%
• Loja que dobrou as vendas online  
• Profissional que triplicou os clientes

Qual seu segmento? 🎯`;

    return this.formatResponse(examplesResponse, "sara_examples");
  }
...
`;
      ...
      console.log('🔄 Usando fallback para pergunta sobre preços');
      
      const priceResponse = `Ótima pergunta! 💰 Nossos preços são super justos:
...
💡 **Dica:** Uma landing page de R$ 800 pode gerar R$ 10.000 em vendas no primeiro mês!

Que tipo de projeto você precisa?`;

      return this.formatResponse(priceResponse, "sara_price_fallback");
    }
  }
