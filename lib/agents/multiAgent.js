...
export class MultiAgentSalesSystem {
  constructor() {
    ...
    this.saraAI = new SaraAI(); // Nova Sara AI integrada
  }

  // Método novo usando Sara AI
  async processWithSaraAI(userMessage, userInfo = {}) {
    try {
      return await this.saraAI.processMessage(userMessage, userInfo);
    } catch (error) {
      console.error("Erro na Sara AI:", error);
      return {
        success: false,
        response: "Desculpe, houve um problema técnico. Entre em contato diretamente conosco.",
        error: error.message
      };
    }
  }
